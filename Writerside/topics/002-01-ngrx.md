...
<note>
Il est possible de créer toutes les actions d'un seul coup avec cette syntaxe :

```typescript
createActionGroup({
   source: 'Gestion de mon état',
   events: {
        loadState: emptyProps(),
        updateState: props<User>()
   }
})
```

`props` permet de définir le payload, ce que l'action transporte en termes de données.
</note>
